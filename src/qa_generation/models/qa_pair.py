...
def _validate_metadata_size(
    ...
    try:
        ...
        raise ValueError(
            f"metadata values must be JSON-serializable. Error: {e}"
        ) from e
...
class QAPair(BaseModel):
    ...
    source_location: str | None = Field(
        default=None, description="Section path from the diff report"
    )
    source_change_type: str | None = Field(
        default=None, description="Type of change (text_change, structure_change, etc.)"
    )
    ...
    def has_traceability(self) -> bool:
        ...
        return bool(
            self.source_topic_slug
            and self.source_location
            and self.source_versions
            and self.source_change_type
        )
...
class QueryDistribution(BaseModel):
    ...
    specific: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Simple, specific questions"
    )
    abstract: float = Field(
        default=0.25, ge=0.0, le=1.0, description="Abstract reasoning questions"
    )
    comparative: float = Field(
        default=0.25, ge=0.0, le=1.0, description="Comparative questions"
    )
    ...
    def validate_distribution_sum(self) -> "QueryDistribution":
        ...
        if abs(total - 1.0) >= 0.01:
            raise ValueError(
                ...
                f"Adjust specific={self.specific}, abstract={self.abstract}, "
                f"comparative={self.comparative}"
            )
        return self
...
class FilterConfig(BaseModel):
    ...
    def validate_ranges(self) -> "FilterConfig":
        ...
        if self.min_text_length > self.max_text_length:
            raise ValueError(
                f"min_text_length ({self.min_text_length}) must be <= "
                f"max_text_length ({self.max_text_length})"
            )
        if self.min_similarity > self.max_similarity:
            raise ValueError(
                f"min_similarity ({self.min_similarity}) must be <= "
...
class GeneratorConfig(BaseModel):
    ...
    testset_size: int = Field(
        default=50, ge=1, description="Number of QA pairs to generate"
    )
